...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={cn(
        "h-10 w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--surface)] px-3 text-sm text-[var(--text-strong)] outline-none ring-[var(--brand-400)] transition focus:ring-2",
        props.className,
      )}
    />
  );
...
export function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className={cn(
        "w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--text-strong)] outline-none ring-[var(--brand-400)] transition focus:ring-2",
        props.className,
      )}
    />
  );
...
export function Card({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-[var(--border-subtle)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)]",
...
export function SectionHeader({
  ...
  return (
    <div className="mb-4 flex items-start justify-between gap-4">
      <div>
        <h2 className="text-xl font-semibold tracking-tight text-[var(--text-strong)] md:text-2xl">{title}</h2>
...
export function Badge({
  ...
  return <span className={cn("inline-flex rounded-full px-3 py-1 text-xs font-semibold tracking-wide", toneStyles[tone], className)}>{children}</span>;
}
...
export function StatCard({ label, value, hint }: { label: string; value: string | number; hint?: string }) {
  return (
    <Card className="space-y-2 border-none bg-transparent p-0 shadow-none">
...
export function EmptyState({
  ...
  return (
    <Card className="border-dashed bg-[var(--surface-muted)] text-center shadow-none">
...
export function HelperText({ tone = "neutral", children }: { tone?: "neutral" | "error" | "success"; children: ReactNode }) {
  ...
  return <p className={cn("rounded-lg px-3 py-2 text-sm", tones[tone])}>{children}</p>;
}
